...
$ENV{TEST_NGINX_MEMCACHED_PORT} ||= 11212;
...
--- config
    location /t {
        content_by_lua '
            ...
            assert(memc:set_timeout(2000)) -- 2 sec
            ...
            local session, err = memc:sslhandshake(nil, "test.com", false)
            if err ~= nil then
...
=== TEST 2: verify server certificate failed
--- http_config eval: $::HttpConfig
--- config
    location /t {
        content_by_lua '
            ...
            local session, err = memc:sslhandshake(nil, "test.com", true)
            if err ~= nil then
                ngx.say("failed to connect to memcached: ", err)
                return
            end
